...
function roleHasApprovalOptions(roleId) {
    const role = parseInt(roleId, 10);
...
function setUserSelectValue(select, value) {
    ...
    if (window.jQuery && window.jQuery(select).hasClass('select2-hidden-accessible')) {
        window.jQuery(select).val(nextValue).trigger('change');
        return;
    }
...
function toggleApprovalFields(roleId, selectedIds) {
    ...
    if (!isAllowed) {
        ...
        setUserSelectValue(level2, '');
        level1.classList.remove('is-invalid');
        level2.classList.remove('is-invalid');
        const msg1 = document.querySelector('.validation-msg[data-field="level_1_approver_id"]');
        const msg2 = document.querySelector('.validation-msg[data-field="level_2_approver_id"]');
        if (msg1) {
            msg1.textContent = '';
        }
        if (msg2) {
    ...
    if (selectedIds) {
        ...
        if (selectedIds.level_2_approver_id !== undefined && selectedIds.level_2_approver_id !== null && String(selectedIds.level_2_approver_id) !== '') {
            setUserSelectValue(level2, selectedIds.level_2_approver_id);
        }
    }
}
...
function initUsersFormValidation() {
    ...
    validate.validators.userApproverRequired = function (value, options, key, attributes) {
        ...
        if (!value || String(value).trim() === '') {
            return key === 'level_2_approver_id'
                ? '^Level 2 Approval is required'
                : '^Level 1 Approval is required';
        }
        return null;
    };
    ...
    function clearValidationState() {
        ...
        if (window.jQuery) {
            window.jQuery('#userForm .select2-selection').removeClass('is-invalid');
        }
    }
    ...
    function showErrors(errors) {
        ...
        Object.keys(errors).forEach(function (field) {
            ...
            if (window.jQuery && input) {
                window.jQuery(input).next('.select2-container').find('.select2-selection').addClass('is-invalid');
            }
            if (msg && errors[field]) {
