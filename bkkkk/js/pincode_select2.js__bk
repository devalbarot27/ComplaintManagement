...
function setAddressAutoFields(form, data) {
  ...
  Object.keys(mapping).forEach(function (field) {
    ...
    const msg = form.querySelector(
      '.validation-msg[data-field="' + field + '"]',
    );
    if (msg) {
      msg.textContent = "";
    }
...
function setPincodeSelect2(form, pincodeSelectId, data) {
  ...
  const postcode =
    data && data.pincode != null ? String(data.pincode).trim() : "";
  ...
  if (postcode !== "") {
    ...
    const state_code = data.state_code != null ? String(data.state_code).trim() : "";
    const text =
      city !== "" || state !== ""
        ? postcode + " - " + city + ", " + state
        : postcode;
