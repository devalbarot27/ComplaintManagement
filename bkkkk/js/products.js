function initProductFormValidation() {
    ...
    const constraints = {
        ...
        level_1_approval_price: {
            presence: { allowEmpty: false, message: '^Level 1 Approval Price is required' },
            format: {
                pattern: /^-?\d+(\.\d+)?$/,
                message: '^Level 1 Approval Price must be numeric'
            }
        },
        level_2_approval_price: {
            presence: { allowEmpty: false, message: '^Level 2 Approval Price is required' },
            format: {
                pattern: /^-?\d+(\.\d+)?$/,
                message: '^Level 2 Approval Price must be numeric'
            }
        },
        mc: {
            format: {
    ...
    function getActiveConstraints(values) {
        ...
        ['mc', 'vc', 'fc', 'otcode'].forEach(function (field) {
            if (values[field] === '' || values[field] == null) {
                delete activeConstraints[field];
            }
        });
        if (String(values.order_type || '') === '2') {
            delete activeConstraints.level_1_approval_price;
            delete activeConstraints.level_2_approval_price;
        }
    ...
    if (orderTypeInput) {
        orderTypeInput.addEventListener('change', function () {
            syncProductApprovalPriceFields();
        });
    }
    ...
    ['level_1_approval_price', 'level_2_approval_price', 'mc', 'vc', 'fc', 'cos'].forEach(function (fieldName) {
        initNumericOnlyInput(form.querySelector('[name="' + fieldName + '"]'));
    });
}

function syncProductApprovalPriceFields() {
    const form = document.getElementById('productForm');
    if (!form) {
        return;
    }
    const orderTypeInput = form.querySelector('[name="order_type"]');
    const spares = String((orderTypeInput && orderTypeInput.value) || '') === '2';
    form.querySelectorAll('.js-units-approval-price-field').forEach(function (el) {
        el.hidden = spares;
    });
    if (spares) {
        ['level_1_approval_price', 'level_2_approval_price'].forEach(function (fieldName) {
            const input = form.querySelector('[name="' + fieldName + '"]');
            const msg = form.querySelector('.validation-msg[data-field="' + fieldName + '"]');
            if (input) {
                input.classList.remove('is-invalid');
...
function fillProductForm(record) {
    ...
    const fields = [
        'dpst', 'product_group', 'tplcode', 'tpldesc',
