function validateInstalledBaseServiceLogDraftForm(form) {
    const errors = {};
    ...
    requireField('warranty_chargeable', 'Complaint Category is required');
...
function showInstalledBaseServiceLogDraftErrors(form, errors) {
    ...
    if (typeof ibServiceLogShowPartReplacementErrors === 'function') {
        ibServiceLogShowPartReplacementErrors(form, errors);
    }
}

function ibServiceLogRedirectAfterDraftSave() {
    window.location.replace('installed_base.php?service_log_draft_added=1');
}

function initInstalledBaseServiceLogDraftSave() {
    ...
    draftBtn.addEventListener('click', function () {
        ...
        const errors = validateInstalledBaseServiceLogDraftForm(form);
        showInstalledBaseServiceLogDraftErrors(form, errors);
        ...
        if (!installedBaseId || parseInt(installedBaseId, 10) <= 0) {
            showInstalledBaseServiceLogDraftErrors(form, { installed_base_id: ['Installed base record is required'] });
            return;
        }
        ...
        $.ajax({
            url: 'api/installed_base_service_log_draft_create.php',
            type: 'POST',
            data: $(form).serialize(),
            dataType: 'json'
        })
            .done(function () {
                ibServiceLogRedirectAfterDraftSave();
            })
            .fail(function (xhr) {
