...
$stmt = $obconn->prepare('
    SELECT sl.*, ib.customer_name, ib.dealer_name, ib.machine_model AS ib_machine_model, ib.machine_model_code
...
$machineModelLabel = installed_base_machine_model_label([
    'machine_model_code' => $record['machine_model_code'] ?? '',
    'machine_model' => $record['ib_machine_model'] ?? $record['machine_model'] ?? '',
]);

if ($machineModelLabel === '-') {
    $machineModelLabel = service_log_display_value($record['machine_model']);
}
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    <div class="main-wrapper" id="mainWrapper">
        ...
        <div class="content">
            ...
            <div class="card border-1 shadow-sm mb-3">
                ...
                <div class="card-body row g-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <?php echo htmlspecialchars(service_log_display_value($record['fab_number'])); ?>
                    </div>
                    <div class="col-md-4">
            ...
            <div class="card border-1 shadow-sm mb-3">
                <div class="card-header bg-white"><strong>Remaining Consumables Details</strong></div>
                <div class="card-body row g-3">
                    <?php foreach (service_log_remaining_consumable_fields() as $consumable) {
                        ...
                        ?>
                    <div class="col-md-6">
                        <strong><?php echo htmlspecialchars($consumable['label']); ?> Remaining Date:</strong>
                        <?php echo service_log_format_date($record[$dateField] ?? null); ?>
                    </div>
                    <div class="col-md-6">
                        <strong><?php echo htmlspecialchars($consumable['label']); ?> Remaining Hours:</strong>
                        <?php echo htmlspecialchars(service_log_display_value($record[$hoursField] ?? null)); ?>
                    </div>
                    <?php } ?>
                </div>
            </div>
