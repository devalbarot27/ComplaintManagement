...
function login_fetch_user(PDO $conn, string $username): ?array
{
    $sql = "
        SELECT usr_name, password, display_name, email, mobile
        FROM userpass
        WHERE TRIM(usr_name) = :usr_name
        LIMIT 1
    "; //  AND TRIM(usr_desig) IN ('0', '1')
...
{
    $sql = "
        SELECT username, name, email, password, mobile_number
        FROM user_master
...
print_($user); die();
    ...
    return login_fetch_user($dpconn, $username);
}
...
function login_attempt_remember(PDO $dpconn, PDO $obconn): bool
{
    ...
    $user = login_fetch_user_auth($dpconn, $obconn, (string) $payload['usr_name']);
    if ($user === null) {
        login_clear_remember_cookie();
        return false;
    }
...
{
    $user = login_fetch_user_auth($dpconn, $obconn, $username);
    if ($user === null) {
...
function login_user_from_otp_session(PDO $dpconn, PDO $obconn): ?array
{
    ...
    return login_fetch_user_auth($dpconn, $obconn, $usrName);
}
