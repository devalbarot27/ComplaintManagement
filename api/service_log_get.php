...
$fromComplaintFlow = $complaintId > 0;
...
if ($fromComplaintFlow) {
    complaint_service_log_require_assigned_access($obconn, $complaintId);
    complaint_service_log_ensure_schema($obconn);

    if (!complaint_service_log_is_linked($obconn, $complaintId, $id)) {
        http_response_code(404);
        echo json_encode(['error' => 'Record not found.']);
        exit;
    }
} elseif (!after_market_user_can_access_record($obconn, 'service_logs', $id)) {
    http_response_code(404);
    echo json_encode(['error' => 'Record not found.']);
    exit;
}
