...
require_once __DIR__ . '/includes/order_cart_schema.php';
...
class orderClass
{
    ...
    public function __construct($obconn, $dpconn)
    {
        ...
        order_approval_ensure_schema($this->obconn);
    }

    // APPROVAL MODULE: Added Level 1 and Level 2 cart approval logic
    private function orderApprovalSubmitBlockMessage(): ?string
    {
    ...
    public function getPrice_BK090826()
    {
    ...
    public function getPrice()
    {
        ...
        $stmt = $this->obconn->prepare("
        SELECT cos, dealer_price, level_1_approval_price, level_2_approval_price
        FROM product_master_vayu
        WHERE tplcode = :item
        AND order_type =:orderType
        LIMIT 1");
        ...
        if ($row) {
            // PRICE TYPE: Added for CLP / Level 1 / Level 2 Approval Price
            $priceType = cart_normalize_price_type($_POST['price_type'] ?? 'clp');
            $prices = cart_product_price_map($row);
            echo json_encode([
                'status' => true,
                'price'  => htmlspecialchars($prices[$priceType], ENT_QUOTES, 'UTF-8'),
    ...
    public function addItemCart()
    {
        try {
            ...
            $getDesc = $this->obconn->prepare("SELECT tpldesc,dpst,cos,dealer_price,level_1_approval_price,level_2_approval_price FROM product_master_vayu WHERE tplcode=:tplcode and order_type =:orderType");
            ...
            $clpPrice = round(cart_numeric_price($fetDesc['cos'] ?? null), 2);
            $isSpares = ((int) $orderType === 2);
            if ($isSpares) {
                $price = $clpPrice;
                if ($price <= 0) {
                    return json_encode('Price should not be accepted.');
                }
                $priceType = 'clp';
            } else {
                $price = round(cart_numeric_price($_POST['price'] ?? null), 2);
                if ($price <= 0) {
                    return json_encode('Price should not be accepted.');
                }
                if ($clpPrice > 0 && $price > $clpPrice) {
                    return json_encode('Price should not be accepted.');
                }
                if (cart_dealer_price_below_l2($fetDesc, $price)) {
                    return json_encode('Price should not be accepted.');
                }
                $priceType = cart_price_type_from_unit_price($fetDesc, $price);
            }
            $stmt = $this->obconn->prepare("SELECT id, qty FROM tbl_vayu_cartitems WHERE created_by = :createdBy AND status = 0 AND item_code = :item_code AND order_type = :orderType");
            ...
            $dpst = $fetDesc['dpst'] ?? '-';
            // APPROVAL MODULE: Order-level approval starts after Submit for Approval
            $approvalStatus = 'not_required';
            $cartItemId = 0;
            if (!$item) {
                $totalAmount = $qty * $price;
                $insert = $this->obconn->prepare("INSERT INTO tbl_vayu_cartitems(item_code,item_name,price,qty,total_amount,created_by,dpst,order_type,price_type,approval_status)VALUES(:item_code,:item_name,:price,:qty,:total_amount,:created_by,:dpst,:order_type,:price_type,:approval_status) RETURNING id");
                ...
                $insert->bindValue(':price_type', $priceType);
                $insert->bindValue(':approval_status', $approvalStatus);
                if (!$insert->execute()) {
                    return 0;
                }
                $cartItemId = (int) $insert->fetchColumn();
            } else {
                $updatedQty = $item['qty'] + $qty;
                $totalAmount = $updatedQty * $price;
                $update = $this->obconn->prepare("UPDATE tbl_vayu_cartitems SET qty = :qty,price = :price,total_amount = :totalAmount,price_type = :price_type WHERE created_by = :createdBy AND status = 0 AND item_code = :item_code AND dpst=:dpst AND order_type =:orderType");
    ...
    public function addItemCart_BK090826()
    {
        try {
            $item_code = filter_input(INPUT_POST, 'item', FILTER_SANITIZE_SPECIAL_CHARS);
            $qty = filter_input(INPUT_POST, 'qty', FILTER_VALIDATE_FLOAT);
    ...
    private function freightPercentage(): float
    {
        return 4;
    }
    ...
    private function computeCartLineGstAmounts(float $unitPrice, float $qty): array
    {
        ...
        $cgstRate = 9.0;
        $sgstRate = 9.0;
        $cgstAmount = round($lineBase * $cgstRate / 100, 2);
        $sgstAmount = round($lineBase * $sgstRate / 100, 2);
        $lineGst = round($cgstAmount + $sgstAmount, 2);
        $lineTotal = round($lineBase + $lineGst + $freight, 2);
    ...
    private function renderCartGstBreakdownHtml(float $cgst, float $sgst, ?float $totalGst = null): string
    {
        if ($totalGst === null) {
            $totalGst = round($cgst + $sgst, 2);
        }

        return '<div class="cart-gst-breakdown">'

            . '<span class="cart-gst-line cart-gst-line--total">' . $this->formatCartMoney($totalGst) . '</span>'
            . '</div>';
    }

    private function formatCartMoney(float $amount): string
    {
    ...
    public function getCartItems()
    {
        ...
        if ($chkItem->rowCount() == 0) {
            ...
            <div class="cart-empty-state">
                <div class="cart-empty-state__icon"><i class="bi bi-cart3"></i></div>
                <h4 class="cart-empty-state__title">Your cart is empty</h4>
                <p class="cart-empty-state__text">Select a product above and click <strong>Add Item</strong> to start building your order.</p>
            </div>';
        }
    ...
    <table class="cart-table" id="cartTable">
        <thead>
            <tr>
                <th class="cart-col-code">Item Code</th>
                <th class="cart-col-desc">Item Description</th>
                <th class="cart-col-money">Unit Price</th>
                <th class="cart-col-qty">Qty</th>
                <th class="cart-col-total-price">Total Price</th>
                <th class="cart-col-gst">GST (18%)<span class="cart-gst-hint">CGST (9%) + SGST (9%)</span></th>
                <th class="cart-col-freight">Freight (' . $freightPercentage . '%' . ')</th>
                <th class="cart-col-money">Total Amount</th>
        ...
        <tr class="cart-item-row" data-cart-id="' . $cartId . '" data-unit-price="' . htmlspecialchars((string) $unitPrice, ENT_QUOTES, 'UTF-8') . '">
            <td class="cart-col-code"><span class="cart-item-code">' . htmlspecialchars($row['item_code']) . '</span></td>
            <td class="cart-col-desc">' . htmlspecialchars($row['item_name']) . '</td>
            <td class="cart-col-money cart-money" id="idUnitPrice' . $cartId . '">' . $this->formatCartMoney($unitPrice) . '</td>
            <td class="cart-col-qty">
                <input type="text" value="' . htmlspecialchars((string) $row['qty']) . '" id="idQty' . $cartId . '" class="cart-qty-input" oninput="recalculateCartLine(' . $cartId . ')" onKeyup="updatePrice(\'' . $cartId . '\')">
            </td>
            <td class="cart-col-total-price cart-money" id="idLineTotalPrice' . $cartId . '">' . $this->formatCartMoney($amounts['line_base']) . '</td>
            <td class="cart-col-gst" id="idGst' . $cartId . '">'
                . $this->renderCartGstBreakdownHtml($amounts['cgst_amount'], $amounts['sgst_amount'], $amounts['line_gst']) .
                '</td>
            <td class="cart-col-freight cart-money" id="idFreight' . $cartId . '">' . $this->formatCartMoney($amounts['freight']) . '</td>
            <td class="cart-col-money cart-money cart-money--total" id="idTotal' . $cartId . '">' . $this->formatCartMoney($amounts['line_total']) . '</td>
            <td class="cart-col-action">
                <div class="cart-action-group">
                    <button type="button" class="cart-action-btn cart-action-btn--breakup" onclick="openPriceBreakup(' . $cartId . ')" title="Price Breakup">
                        <i class="bi bi-receipt"></i>
                    </button>
                    <button type="button" class="cart-action-btn cart-action-btn--delete removeItem" onclick="deleteCartItem(' . $cartId . ')" data-id="' . $cartId . '" title="Remove item">
        ...
        $totalPriceSum = round($totalPriceSum, 2);
        $totalGstSum = round($totalGstSum, 2);
        $totalFreightSum = round($totalFreightSum, 2);
        $totalAmountSum = round($totalAmountSum, 2);

        $html .= '
        </tbody>
        <tfoot class="cart-table__footer">
        <tr class="cart-summary-row cart-summary-row--grand">
            <td colspan="4" class="cart-summary-label cart-summary-label--grand">Grand Total</td>
            <td class="cart-col-money cart-summary-value" id="cartTotalPrice" data-cart-total-price="' . htmlspecialchars((string) $totalPriceSum, ENT_QUOTES, 'UTF-8') . '">
                ' . $this->formatCartMoney($totalPriceSum) . '
            </td>
            <td class="cart-col-money cart-summary-value cart-summary-value--gst" id="cartTotalGst" data-cart-total-gst="' . htmlspecialchars((string) $totalGstSum, ENT_QUOTES, 'UTF-8') . '">
                ' . $this->formatCartMoney($totalGstSum) . '
            </td>
            <td class="cart-col-money cart-summary-value" id="cartTotalFreight" data-cart-total-freight="' . htmlspecialchars((string) $totalFreightSum, ENT_QUOTES, 'UTF-8') . '">
                ' . $this->formatCartMoney($totalFreightSum) . '
            </td>
            <td class="cart-col-money cart-summary-value cart-summary-value--grand" id="cartTotalAmount" data-cart-total-amount="' . htmlspecialchars((string) $totalAmountSum, ENT_QUOTES, 'UTF-8') . '">
                ' . $this->formatCartMoney($totalAmountSum) . '
            </td>
            <td class="cart-col-action">
                <button type="button" class="cart-action-btn cart-action-btn--breakup cart-action-btn--breakup-lg" onclick="openOrderPriceBreakup()" title="Order Price Breakup">
    ...
    public function getCartItems_bk()
    {
 ...
 <button type="button"
                            ...
                            onclick="openPriceBreakup(' . (int) $row['id'] . ')"
                            title="Price Breakup" style="background-color:#F44611">
    ...
    public function deleteItem()
    {
        ...
        try {
            ...
            // APPROVAL MODULE: Added Level 1 and Level 2 cart approval logic
            if ($id) {
                order_approval_cancel_for_cart_item($this->obconn, (int) $id, $this->userId);
            }
    ...
    public function deleteItem_BK090826()
    {
    ...
    private function parseGstTaxRate(?string $taxCode): ?float
    {
        $taxCode = trim((string) $taxCode);
        if ($taxCode === '' || !preg_match('/GSTAG(\d+)/', $taxCode, $matches)) {
            return null;
        }

        $rate = (float) $matches[1];

        return $rate > 0 && $rate <= 28 ? $rate : null;
    }

    private function formatPriceBreakupCurrency($amount): string
    {
    ...
    private function formatPriceBreakupPercent(?float $rate): string
    {
        if ($rate === null) {
            return 'N/A';
        }
    ...
    private function resolveCartPriceBreakupTaxRates(
        string $itemCode,
        ...
        $orderCategoryId = (int) $orderCategory;

        // Override for order categories 4 & 6
        if (in_array($orderCategoryId, [4, 6], true)) {
            $gstRate = ($state === 'TN') ? 5 : 12;
        } else {
            $gstRate = 18;
        }
    ...
    private function fetchPriceBreakupCustomerContext(): array
    {
        $customerStmt = $this->obconn->prepare("
            ...
            LIMIT 1
        ");
        $customerStmt->bindValue(':cuno', $this->customer_code, PDO::PARAM_STR);
        $customerStmt->execute();
        $customer = $customerStmt->fetch(PDO::FETCH_ASSOC) ?: [];

        return [
            'country' => trim((string) ($customer['country'] ?? 'IND')),
            'state' => 'TN',
        ];
    }
    ...
    /**
     * @return array{
     *     price_inclusive: float,
     *     price_before_gst: ?float,
     *     cgst_rate: ?float,
     *     sgst_rate: ?float,
     *     cgst_amount: ?float,
     *     sgst_amount: ?float
     * }
     */
    private function computeLinePriceBreakup(array $item, string $orderCategory, array $customerContext): array
    {
        $amounts = $this->computeCartLineGstAmounts(
            (float) ($item['price'] ?? 0),
            (float) ($item['qty'] ?? 0)
        );

        return [
            'price_inclusive' => $amounts['line_total'],
            'price_before_gst' => $amounts['line_base'],
            'freight' => $amounts['freight'],
            'line_gst' => $amounts['line_gst'],
    ...
    private function formatPriceBreakupResponse(array $breakup, array $extra = []): string
    {
        return json_encode(array_merge([
            'status' => true,
            'price_before_gst' => $this->formatPriceBreakupCurrency($breakup['price_before_gst'] ?? null),
            'cgst_percent' => $this->formatPriceBreakupPercent($breakup['cgst_rate'] ?? null),
            'cgst_amount' => $this->formatPriceBreakupCurrency($breakup['cgst_amount'] ?? null),
            'sgst_percent' => $this->formatPriceBreakupPercent($breakup['sgst_rate'] ?? null),
            'sgst_amount' => $this->formatPriceBreakupCurrency($breakup['sgst_amount'] ?? null),
            'freight_amount' => $this->formatPriceBreakupCurrency($breakup['freight'] ?? null),
            'total_price' => $this->formatPriceBreakupCurrency($breakup['price_inclusive'] ?? null),
        ], $extra));
    }

    /**
     * Display-only price breakup for a cart line. Cart price is GST-inclusive;
     * base price and tax components are derived for display only (no DB writes).
     */
    public function getCartPriceBreakup()
    {
        try {
            $id = filter_input(INPUT_POST, 'id', FILTER_VALIDATE_INT);
            $orderCategory = trim((string) ($_POST['orderCategory'] ?? ''));
            ...
            $cartStmt = $this->obconn->prepare("
                SELECT id, item_code, item_name, price, qty, total_amount
                FROM tbl_vayu_cartitems
                WHERE id = :id
            ...
            $cartStmt->bindValue(':createdBy', $this->userId, PDO::PARAM_STR);
            $cartStmt->execute();
            ...
            $breakup = $this->computeLinePriceBreakup($item, $orderCategory, $customerContext);

            return $this->formatPriceBreakupResponse($breakup, [
    ...
    /**
     * Display-only consolidated price breakup for all cart items combined.
     */
    public function getCartOrderPriceBreakup()
    {
        try {
            $orderCategory = trim((string) ($_POST['orderCategory'] ?? ''));
            $customerContext = $this->fetchPriceBreakupCustomerContext();
            ...
            foreach ($items as $item) {
                $lineBreakup = $this->computeLinePriceBreakup($item, $orderCategory, $customerContext);
                $totals['price_inclusive'] += (float) ($lineBreakup['price_inclusive'] ?? 0);
                $totals['price_before_gst'] += (float) ($lineBreakup['price_before_gst'] ?? 0);
                $totals['freight'] += (float) ($lineBreakup['freight'] ?? 0);
                $totals['line_gst'] += (float) ($lineBreakup['line_gst'] ?? 0);
                $totals['cgst_amount'] += (float) ($lineBreakup['cgst_amount'] ?? 0);
                $totals['sgst_amount'] += (float) ($lineBreakup['sgst_amount'] ?? 0);

                if ($totals['cgst_rate'] === null) {
                    $totals['cgst_rate'] = $lineBreakup['cgst_rate'] ?? null;
                    $totals['sgst_rate'] = $lineBreakup['sgst_rate'] ?? null;
                }
            }

            $totals['price_before_gst'] = round($totals['price_before_gst'], 2);
            $totals['freight'] = round($totals['freight'], 2);
            $totals['line_gst'] = round($totals['line_gst'], 2);
            $totals['cgst_amount'] = round($totals['cgst_amount'], 2);
            $totals['sgst_amount'] = round($totals['sgst_amount'], 2);
            $totals['price_inclusive'] = round($totals['price_inclusive'], 2);

            return $this->formatPriceBreakupResponse($totals, [
    ...
    public function submitCartApi_BK090826()
    {
        ...
        $addrType = $_POST['deliveryAddressType'];
        ...
        if ($addrType == 1) {
            ...
            $getAddr = $this->dpconn->prepare("SELECT * FROM customer_address WHERE adr_code=:addrCode limit 1");
            ...
            if ($getAddr->rowCount() > 0) {
                ...
                $emailValue =  null; // Added new field 01-07-26

                $pincodeValue =  ($pincode) ? str_replace(' ', '', $pincode) : 000; // Added new field 01-07-26
            ...
            $stateName = trim($_POST['state'] ?? ''); // Full state name for display/storage

            $state = trim($_POST['state_code'] ?? ''); // Added new field 01-07-26
            if ($state === '') {
                $state = 'TN';
            }
            ...
            // Persist end-customer delivery details; empty delivery_code marks end-customer orders.
        ...
        if ($bearerToken) {
            try {
                ...
                $productStmt = $this->obconn->prepare("SELECT tpldesc, excisable, warehouse, otcode, mc, vc, fc, cos,dealer_price FROM product_master_vayu WHERE tplcode = :tplcode AND dpst = :dpst");
                ...
                foreach ($cartItems as $item) {
                    ...
                    $success = $insertStmt->execute([
                        ...
                        ':edi_delivery_code' => ($addrType == 2) ? '' : $deladdr,
    ...
    public function submitCartApi()
    {
        ...
        $delterms  = trim((string) ($_POST['deliveryTerm'] ?? 'CIF'));
        ...
        // APPROVAL MODULE: Added Level 1 and Level 2 cart approval logic
        $approvalBlockMessage = $this->orderApprovalSubmitBlockMessage();
        if ($approvalBlockMessage !== null) {
            return json_encode([
                'status' => false,
        ...
        if ($bearerToken) {
            try {
                ...
                // APPROVAL MODULE: Added Level 1 and Level 2 cart approval logic
                $cartStmt = $this->obconn->prepare("SELECT item_code,item_name,qty,price,total_amount,price_type FROM tbl_vayu_cartitems WHERE created_by = :createdBy AND status = 0 AND " . order_approval_submittable_sql());
                ...
                $stmt = $this->obconn->prepare("SELECT max(substr(indent_number,7,9)) AS maxindno FROM 
                plexecom_customer_units WHERE areacode = :area AND indent_date >= '01.04.2022'");
                ...
                foreach ($cartItems as $item) {
                    ...
                    $insertStmt = $this->obconn->prepare("INSERT INTO plexecom_customer_units(usr_name,emp_code,cuno,cuname,areacode,pono,indent_category,indent_number,indent_date,order_time,transporter,delterms_code,delivery_date,invaddr,email, pincode,district,deladdr,dpst,tplcode,price,price_type,qty,salestax_code,sessionid,paycode,insby,edi_cuno,seqid,status,aoseries,otcode,warehouse,edi_delivery_date,edi_delivery_code,tpldesc,mc,vc,fc,cos,delivery_code,frtamount,company,adrcode,refno,hsn,state,country,edistatus,edi_date)VALUES(:uname,:emp_code,:cuno,:cname,:area,:pono,:indcat,:indno,current_date,CURRENT_TIME,:trans,:delterms,:deldate,:invaddr,:email,:pincode,:district,:deladdr,:dpst,:tplcode,:price,:price_type,:qty,:taxcode,:sid,:paycode,:insby,:edi_cuno,:seqid,:status,:aoseries,:otcode,:warehouse,:edi_delivery_date,:edi_delivery_code,:tpldesc,:mcval,:vcval,:fcval,:cosval,:shipto,:frtamount,:cmp,:adrcode,:refno,:hsn,:state,:country,:edistatus,:edi_date)");
                    ...
                    $success = $insertStmt->execute([
                        ...
                        ':price_type'        => cart_normalize_price_type($item['price_type'] ?? 'clp'),
                ...
                // APPROVAL MODULE: Added Level 1 and Level 2 cart approval logic
                $deleteStmt = $this->obconn->prepare("DELETE FROM tbl_vayu_cartitems WHERE created_by = :createdBy
                AND status = 0 AND " . order_approval_submittable_sql());
    ...
    /**
     * Create Order:
     * - CLP-only cart: validate, store lines, skip L1/L2, call LN for AO Number.
     * - Any Level 1 / Level 2 price: validate, store lines, start L1/L2; AO waits until approval.
     */
    public function submitCartForApproval()
    {
        date_default_timezone_set('UTC');

        $addrCode  = trim((string) ($_POST['addressCode'] ?? ''));
        $area      = trim((string) ($_POST['area'] ?? ''));
        $indcat1   = trim((string) ($_POST['orderCategory'] ?? ''));
        $deladdr   = strtoupper($addrCode);
        $addrType  = (string) ($_POST['deliveryAddressType'] ?? '1');
        $trans     = trim((string) ($_POST['transporter'] ?? ''));
        $delterms  = 'CIF';
        $paycode   = trim((string) ($_POST['paymentTerm'] ?? ''));
        $pono      = trim((string) ($_POST['pono'] ?? ''));
        $frtamount = (isset($_POST['freightAmount']) && $_POST['freightAmount'] !== '') ? $_POST['freightAmount'] : null;
        $ddate     = trim((string) ($_POST['ddate'] ?? ''));
        $orderType = trim((string) ($_POST['orderType'] ?? ''));
        $indcat    = 'Normal Order';
        $otcode    = $indcat1;
        $aoseries  = ($orderType == '1') ? 'YUU' : 'YUS';
        $cmp       = ($orderType == '1') ? 401 : 490;
        $dpst      = ($orderType == '1') ? 'Y0001' : 'Y0011';
        $cuno      = $this->customer_code;
        $sid       = session_id();
        $state     = 'TN';

        $required = [
            'orderCategory' => $indcat1,
            //'deliveryTerm' => trim((string) ($_POST['deliveryTerm'] ?? '')),
            'paymentTerm' => $paycode,
            'transporter' => $trans,
            'area' => $area,
            'ddate' => $ddate,
        ...
        $requiredMessages = [
            'orderCategory' => 'Please select a order category',
            //'deliveryTerm' => 'Please select a delivery term',
            'paymentTerm' => 'Please select a payment term',
            'transporter' => 'Please select a transporter',
            'area' => 'Please select area',
            'ddate' => 'Please select delivery date',
            'pono' => 'Please enter PO Number',
            'orderType' => 'Please select ordertype',
        ];
        ...
        if ($addrType == '1') {
            ...
            $cuname = trim((string) ($_POST['end_customer_name'] ?? ''));
            $email = trim((string) ($_POST['end_customer_email'] ?? ''));
            $street1 = trim((string) ($_POST['street_1'] ?? ''));
            $street2 = trim((string) ($_POST['street_2'] ?? ''));
            $city = trim((string) ($_POST['city'] ?? ''));
            $stateName = trim((string) ($_POST['state'] ?? ''));
            $state = trim((string) ($_POST['state_code'] ?? ''));
            if ($state === '') {
                $state = 'TN';
            }
            ...
            $endCustomerRequired = [
                'Please enter name' => $cuname,
                'Please enter email' => $email,
                'Please enter street 1' => $street1,
                'Please select pincode' => $pincode,
                'Please enter city' => $city,
                'Please enter district' => $district,
                'Please enter state' => $stateName,
            ];
            foreach ($endCustomerRequired as $message => $value) {
                if ($value === '') {
                    return json_encode([
                        'status' => 'error',
                        'message' => $message,
            ...
            $pincodeValue = ($pincode !== '') ? $pincode : 000;
            $districtValue = ($district !== '') ? $district : null;
            $deladdr = $street1 . '<br> - <br>' .
                $street2 . '<br> - <br>' .
                $city . '<br> - <br>' .
                $stateName . '<br> - <br>' .
        ...
        try {
            ...
            $rs = $this->obconn->prepare("select to_char(current_date,'YYMMDD') as ymd");
            $rs->execute();
            $ymd = $rs->fetch(PDO::FETCH_ASSOC)['ymd'];

            $rs = $this->obconn->prepare("select nextval('dp_spares') as slno");
            $rs->execute();
            $slno = str_pad($rs->fetch(PDO::FETCH_ASSOC)['slno'], 4, '0', STR_PAD_LEFT);
            $refno = 'E/UNITS/' . $ymd . $slno;

            $cartStmt = $this->obconn->prepare('
                SELECT id, item_code, item_name, qty, price, total_amount, price_type, dpst, order_type
                FROM tbl_vayu_cartitems
                WHERE created_by = :createdBy
                  AND status = 0
                ORDER BY id ASC
            ');
            $cartStmt->execute([':createdBy' => $this->userId]);
            $cartItems = $cartStmt->fetchAll(PDO::FETCH_ASSOC);
            if (empty($cartItems)) {
                throw new Exception('Cart is empty.');
            }

            $cartOrderType = (string) (int) ($cartItems[0]['order_type'] ?? 0);
            foreach ($cartItems as $cartLine) {
                if ((string) (int) ($cartLine['order_type'] ?? 0) !== $cartOrderType) {
                    throw new Exception('Items with different order types cannot be submitted together.');
                }
            }
            if ($cartOrderType !== (string) (int) $orderType) {
                throw new Exception('Order type does not match the items in the cart. Please select the same order type or clear the cart.');
            }

            $stmt = $this->obconn->prepare("
                SELECT max(substr(indent_number,7,9)) AS maxindno
                FROM plexecom_customer_units
                WHERE areacode = :area AND indent_date >= '01.04.2022'
            ");
            ...
            $customerStmt = $this->obconn->prepare('
                SELECT cm.adr_code, cm.country, ca.custaddr
                FROM customer_master cm
                ...
                WHERE cm.cuno = :cuno
            ');
            $customerStmt->execute([':cuno' => $cuno]);
            $customer = $customerStmt->fetch(PDO::FETCH_ASSOC);
            ...
            $productStmt = $this->obconn->prepare('
                SELECT tpldesc, excisable, warehouse, otcode, mc, vc, fc, cos, dealer_price
                FROM product_master_vayu
                WHERE tplcode = :tplcode AND dpst = :dpst
            ');
            $sparesProductStmt = $this->obconn->prepare('
                SELECT tpldesc, excisable, warehouse, otcode, mc, vc, fc, cos, dealer_price
                FROM product_master_vayu
                WHERE tplcode = :tplcode AND order_type = :order_type
                LIMIT 1
            ');
            ...
            $insertStmt = $this->obconn->prepare("
                INSERT INTO plexecom_customer_units (
                    usr_name, emp_code, cuno, cuname, areacode, pono, indent_category, indent_number,
                    indent_date, order_time, transporter, delterms_code, delivery_date, invaddr,
                    email, pincode, district, deladdr, dpst, tplcode, price, price_type, qty,
                    salestax_code, sessionid, paycode, insby, edi_cuno, seqid, status, aoseries,
                    otcode, warehouse, edi_delivery_date, edi_delivery_code, tpldesc, mc, vc, fc, cos,
                    delivery_code, frtamount, company, adrcode, refno, hsn, state, country, edistatus, edi_date
                ) VALUES (
                    :uname, :emp_code, :cuno, :cname, :area, :pono, :indcat, :indno,
                    current_date, CURRENT_TIME, :trans, :delterms, :deldate, :invaddr,
                    :email, :pincode, :district, :deladdr, :dpst, :tplcode, :price, :price_type, :qty,
                    :taxcode, :sid, :paycode, :insby, :edi_cuno, :seqid, :status, :aoseries,
                    :otcode, :warehouse, :edi_delivery_date, :edi_delivery_code, :tpldesc, :mcval, :vcval, :fcval, :cosval,
                    :shipto, :frtamount, :cmp, :adrcode, :refno, :hsn, :state, :country, :edistatus, :edi_date
                )
            ");
            ...
            foreach ($cartItems as $item) {
                $tplcode = $item['item_code'];
                if ((int) $cartOrderType === 2) {
                    $sparesProductStmt->execute([
                        ':tplcode' => $tplcode,
                        ':order_type' => (int) $cartOrderType,
                    ]);
                    $product = $sparesProductStmt->fetch(PDO::FETCH_ASSOC);
                } else {
                    $productStmt->execute([
                        ':tplcode' => $tplcode,
                        ':dpst' => $dpst,
                ...
                $seqStmt->execute();
                $seqid = $seqStmt->fetchColumn();
                $hsnStmt->execute([':tplcode' => $tplcode]);
                $hsn = $hsnStmt->fetchColumn();
                $taxColumn = ($country == 'IND' && $state == 'TN') ? 'sgst' : 'igst';
                $taxStmt = $this->obconn->prepare("SELECT {$taxColumn} AS taxcode FROM gst_hsn WHERE replace(hsn,':','') = :hsn AND company = :company");
                $taxStmt->execute([
                ...
                $cartItemId = (int) ($item['id'] ?? 0);
                if ($cartItemId > 0) {
                    order_approval_cancel_for_cart_item($this->obconn, $cartItemId, $this->userId);
                }
            }
            ...
            $deleteStmt = $this->obconn->prepare('
                ...
                WHERE created_by = :createdBy
                  AND status = 0
            ');
            $deleteStmt->execute([':createdBy' => $this->userId]);

            $this->obconn->commit();

            if (!$needsApproval) {
                $aoError = $this->generateAoNumberAfterApproval($refno);
                if ($aoError !== null) {
                    return json_encode([
                        'status' => 'success',
                        'order_no' => $refno,
                        ...
                        'ln_requested' => false,
                        'message' => $aoError . ' The order is saved. You can use Re-Push from Recent Orders.',
                    ], JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT);
                }
                ...
                return json_encode([
                    ...
                    'needs_approval' => false,
                    'ln_requested' => true,
                    'message' => 'Order submitted successfully. AO Number generation has been requested from LN.',
                ], JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT);
            }
    ...
    public function submitCart()
    {
        try {
            ...
            WHERE created_by = :createdBy
            AND status = 0
            AND " . order_approval_submittable_sql() . "
        ");
        ...
        ");
            ...
            $adrcode = $customer['adr_code'];
            $country = trim($customer['country']);
            $hasEndCustomerAddress = $isEndCustomer && ( // Added new field 01-07-26
            ...
            // Invoice address always stays as dealer/customer master address.
            $invaddr = pg_escape_string($customer['custaddr']);

            if ($hasEndCustomerAddress) { // Added new field 01-07-26
                $deladdr = $street1 . '<br> - <br>' . // Added new field 01-07-26
        ...
        ");
            ...
            foreach ($cartItems as $item) {
                ...
                $success = $insertStmt->execute([
                    ...
                    ':warehouse'         => '257',
                    ':edi_delivery_date' => date('d.m.Y'),
                    ':edi_delivery_code' => $isEndCustomer ? '' : $deladdr,
    ...
    public function submitCart_BK090826()
    {
        try {
...
*/
            ...
            if (!empty($search)) {
                $where = "AND (
                    ...
                    OR COALESCE(p.cuname, '') ILIKE :search
                )";
                $params[':search'] = "%{$search}%";
            }
            ...
            $totalQry = $this->dpconn->prepare("
                SELECT COUNT(*) FROM (
                    ...
                    WHERE p.company != 600 AND {$cunoFilter}
                    GROUP BY p.ordno
                ) x
            ");
            if (!$seeAll) {
            ...
            $sql = "
                SELECT
                    p.cuno,
                    MAX(p.cuname) AS cuname,
                    p.ordno,
                    MAX(p.orddt) AS orddt,
                    MAX(COALESCE(p.pono, '')) AS pono,
                    MAX(COALESCE(p.indentno, '')) AS indentno
                FROM pendingordersnew p
                WHERE {$baseWhere}
                GROUP BY p.cuno, p.ordno
            ...
            foreach ($rows as $row) {
                ...
                $data[] = [
                    ...
                    'order_date'       => !empty($row['orddt']) ? date('d-m-Y', strtotime($row['orddt'])) : '-',
                    'customer'         => htmlspecialchars($customerLabel, ENT_QUOTES, 'UTF-8'),
                    'po_number'        => trim((string) ($row['pono'] ?? '')) !== ''
                        ? htmlspecialchars(trim((string) $row['pono']), ENT_QUOTES, 'UTF-8')
                        : '-',
                    'invoice_address'  => $this->formatPendingOrderTextAddress($plexecom['invaddr'] ?? null),
                    'payment_terms'    => $this->displayOrDash($plexecom['payment_terms'] ?? null),
                    'delivery_terms'   => $this->displayOrDash($plexecom['delivery_terms'] ?? null),
                    'delivery_address' => $this->formatPendingOrderTextAddress($plexecom['deladdr'] ?? null),
                    'action'           => $actionCell,
                ];
            }
     ...
     */
    public function getPendingOrderPriceBreakup()
    {
        try {
            $ordno = trim((string) ($_POST['ordno'] ?? ''));
            ...
            $stmt = $this->dpconn->prepare("
                SELECT itemcode, itemdesc, qty, unitvalue
                ...
                ORDER BY posno
            ");
            $stmt->bindValue(':cuno', $this->customer_code, PDO::PARAM_STR);
            $stmt->bindValue(':ordno', $ordno, PDO::PARAM_STR);
            $stmt->execute();
            ...
            if (!$items) {
                return json_encode([
                    'status' => false,
                    'message' => 'No pending order lines found.',
                ]);
            }
            ...
            foreach ($items as $item) {
                $amounts = $this->computeCartLineGstAmounts(
                    (float) ($item['unitvalue'] ?? 0),
                    (float) ($item['qty'] ?? 0)
                );

                $totals['price_inclusive'] += (float) $amounts['line_total'];
                $totals['price_before_gst'] += (float) $amounts['line_base'];
                $totals['freight'] += (float) $amounts['freight'];
                $totals['line_gst'] += (float) $amounts['line_gst'];
                $totals['cgst_amount'] += (float) $amounts['cgst_amount'];
                $totals['sgst_amount'] += (float) $amounts['sgst_amount'];

                if ($totals['cgst_rate'] === null) {
                    $totals['cgst_rate'] = $amounts['cgst_rate'] ?? null;
                    $totals['sgst_rate'] = $amounts['sgst_rate'] ?? null;
                }
            }
    ...
    private function displayOrDash($value): string
    {
     ...
     */
    private function fetchPendingOrderPlexecomDetails(array $orderNumbers): array
    {
        ...
        $sql = "
            SELECT DISTINCT ON (TRIM(a.order_number))
                ...
                TRIM(COALESCE(a.refno, '')) AS refno,
                a.invaddr,
                a.deladdr,
                COALESCE(NULLIF(TRIM(e.pay_desc), ''), NULLIF(TRIM(a.paycode), '')) AS payment_terms,
                COALESCE(NULLIF(TRIM(c.delivery_term), ''), NULLIF(TRIM(a.delterms_code), '')) AS delivery_terms
            FROM plexecom_customer_units a
            LEFT JOIN tbl_vayu_delivery_term c
                ON a.delterms_code = c.delivery_code::varchar
            LEFT JOIN spp_payterm_master e
                ON a.paycode = e.pay_code::varchar
            WHERE a.cuno = :cuno
              AND TRIM(a.order_number) IN (" . implode(', ', $placeholders) . ")
            ORDER BY TRIM(a.order_number), a.indent_date DESC, a.oid DESC
        ";
        ...
        foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
            $map[trim((string) $row['order_number'])] = $row;
        }

        return $map;
    }

    private function formatPendingOrderTextAddress($value): string
    {
        $value = trim((string) $value);
        if ($value === '') {
            return '-';
        ...
        $textLen = function_exists('mb_strlen') ? mb_strlen($fullText) : strlen($fullText);
        if ($textLen > 80) {
            $display = (function_exists('mb_substr') ? mb_substr($fullText, 0, 77) : substr($fullText, 0, 77)) . '...';
        } else {
            $display = $fullText;
        }

        return '<span title="' . htmlspecialchars($fullText, ENT_QUOTES, 'UTF-8') . '">'
            . htmlspecialchars($display, ENT_QUOTES, 'UTF-8')
            . '</span>';
    }
    // End of New Pending Order List
    ...
    public function getPendingOrderList()
    {
        try {
            ...
            foreach ($rows as $row) {
                ...
                $getRef = $this->obconn->prepare("SELECT refno FROM plexecom_customer_units WHERE pono =:pono ORDER BY oid desc limit 1");
                $getRef->bindParam(":pono", $row['pono']);
                $getRef->execute();
                if ($getRef->rowCount() > 0) {
                    $getRefNo = $getRef->fetch(PDO::FETCH_ASSOC);
                    $refNo = $getRefNo['refno'];
                } else {
                    $refNo = '-';
                }
    ...
    public function getOrderAcknowledgeList()
    {
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                $refMap = $this->fetchPendingOrderRefMap(array_column($row, 'ordno'));
                $data[] = [
                    'ref_no'    => $refMap,
                    'po_number' => $row['purno'],
                    'cuno'      => $row['cuno'],
                    'ao_number'      => $row['ordno'],
                    'purno'     => $row['purno'],
                    'ao_date'     => $row['ord_date'],
                    'ord_date'  => date('d-m-Y', strtotime($row['ord_date'])),
                    'lines' => '<a class="btn btn-sm btn-outline-dark" href="order_data.php?order=' . urlencode($row['ordno']) .
    ...
    public function getRecentOrders()
    {
        try {
            ...
            $orderSqlMap = [
                ...
                'approval_status' => 'recent_orders.approval_status',
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $rowData = [
                    ...
                    'approval_status'  => order_approval_status_table_html($row['approval_status'] ?? null),
...

    {
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $orderStatus = $this->resolveRecentOrderStatus($orderNumber, $this->customer_code);
    ...
    public function customer_master()
    {
        ...
        if ($dealer) {
            $stmt->bindParam(':dealer', $dealer, PDO::PARAM_STR);
        } else {
            $stmt->bindParam(':cuno', $this->customer_code, PDO::PARAM_STR);
        }
     ...
     * }
     */
    public function getRecentOrderDetails_BK090826(?string $refno = null): array
    {
...
*/
            ...
            // End-customer orders store email and leave delivery_code empty (or keep email even on older rows).
            $deliveryAddressType = ($storedEmail !== '' || $deliveryCode === '') ? 'end_customer' : 'dealer';
            ...
            $header = [
                ...
                'end_customer_email' => $storedEmail,
            ...
            if ($header['delivery_address_type'] === 'end_customer') {
                ...
                if ($rawAddr === '' || stripos($rawAddr, '<br') === false) {
                    ...
                    // Legacy submitCart stored end-customer lines in invaddr with "<br> - <br>" separators.
                ...
                if ($rawAddr !== '') {
                    ...
                    // Prefer full state name stored in the address blob over state code column.
                    if (trim((string) ($addrParts[3] ?? '')) !== '') {
                        $header['end_customer_state'] = trim((string) $addrParts[3]);
                    }
                    if (trim((string) ($addrParts[4] ?? '')) !== '' && $header['end_customer_district'] === '') {
                        $header['end_customer_district'] = trim((string) $addrParts[4]);
                    }
                    if (trim((string) ($addrParts[5] ?? '')) !== '' && $header['end_customer_pincode'] === '') {
                        $header['end_customer_pincode'] = trim((string) $addrParts[5]);
                    }
                }
            }
           ...
           */
            ...
            $linesSql = "
                SELECT
                    ...
                    a.hsn,
                    a.frtamount,
                    a.price_type as price_type_new
                FROM plexecom_customer_units AS a
                WHERE a.refno = :refno                 
                ORDER BY a.posno ASC NULLS LAST, a.oid ASC
            "; // AND {$userWhere}
...
*/
            ...
            $taxTotal = 0.0;
            $freightTotal = 0.0;
            $hasLevel1Price = false;
            $hasLevel2Price = false;

            while ($row = $linesStmt->fetch(PDO::FETCH_ASSOC)) {
                $normalizedPriceType = cart_normalize_price_type($row['price_type_new'] ?? '');
                if ($normalizedPriceType === 'level_1') {
                    $hasLevel1Price = true;
                } elseif ($normalizedPriceType === 'level_2') {
                    $hasLevel2Price = true;
                }

                if($row['price_type_new'] == 'clp'){
                    $price_type_new = 'CLP - Customer List Price';
                }else if($row['price_type_new'] == 'dealer'){
                    $price_type_new = 'Dealer Price';
                }else if($row['price_type_new'] == 'level_1'){
                    $price_type_new = 'Level 1 Approval Price';
                }else if($row['price_type_new'] == 'level_2'){
                    $price_type_new = 'Level 2 Approval Price';
                }else{
                    $price_type_new = '-'.$row['price_type_new'];
                }
                $qty = (float) ($row['qty'] ?? 0);
                $price = (float) ($row['price'] ?? 0);
                $lineBase = round($qty * $price, 2);
                ...
                $lines[] = [
                    ...
                    'qty' => $qty,
                    'price' => $price,
                    'price_type' => cart_price_type_label($row['price_type'] ?? 'clp'),
                    'price_type_new' => $price_type_new,
                    'line_total' => $lineBase,
                    'tax_code' => trim((string) ($row['salestax_code'] ?? '')),
                    'tax_amount' => $lineTax,
                    'hsn' => trim((string) ($row['hsn'] ?? '')),
 	  ...
 	  $orderTime = $headerRow['order_time'] ?? null;
            ...
            $header = array_merge(
                $header,
                order_approval_order_level_details($this->obconn, $headerRow, $refno)
            );
            $approvalStatus = strtolower(trim((string) ($headerRow['approval_status'] ?? '')));
    ...
    private function formatRecentOrderAddressText_BK090826($value): string
    {
        $value = trim((string) $value);
        if ($value === '') {
            return '-';
    ...
    ";
        ...
        /**
     * APPROVAL MODULE: Send approved order to LN so AO Number can be generated.
     * order_number stays empty until LN callback updates it.
     */
        public function generateAoNumberAfterApproval(string $refno): ?string
        {
            $refno = trim($refno);
            if ($refno === '') {
            ...
            if (trim((string) ($header['order_number'] ?? '')) !== '') {
                return 'AO Number has already been generated for this order.';
            }
    
            $aoBlock = order_approval_ao_block_reason($header);
            if ($aoBlock !== null) {
                return $aoBlock;
            }
            ...
            try {
                $bearerToken = $this->getBearerTokenLN();
                if ($bearerToken === '') {
                    return 'Unable to authenticate with LN API.';
                }
    
                $xml = $this->buildProcessSalesOrderXmlForRepush($rows);
                if ($xml === '') {
                    return 'Unable to build sales order payload for LN.';
                }
    
                $ionResult = $this->sendIonSalesOrderMessage($xml, $bearerToken);
                if (empty($ionResult['success'])) {
                    return (string) ($ionResult['message'] ?? 'LN request failed.');
                }
                ...
                return 'Unable to generate AO Number. Please use Re-Push later.';
            }
        }
