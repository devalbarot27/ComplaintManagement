...
if (is_file(__DIR__ . '/includes/warranty_claims_helpers.php')) {
...
if (function_exists('warranty_claims_ensure_schema')) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['foc_decision'])) {
    if (!function_exists('foc_claim_apply_decision')) {
        $_SESSION['error_message'] = 'FOC approval is not available.';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['l1_decision'])) {
    if (!function_exists('service_claim_apply_l1_decision')) {
        $_SESSION['error_message'] = 'Service claim approval is not available.';
        header('Location: approvals.php');
        exit;
    }
    $claimId  = (int) ($_POST['claim_id'] ?? 0);
...
// APPROVAL MODULE: Order-level L1 / L2 approval, then AO Number via LN
...
/*
if (defined('FOC_STAGE_PENDING') && defined('FOC_STAGE_APPROVED')) {
try {
    $stmt = $obconn->query("
        SELECT
...
if (defined('SERVICE_CLAIM_L1_PENDING')) {
try {
    $stmt = $obconn->query("
        SELECT
...
// APPROVAL MODULE: Added Level 1 and Level 2 cart approval logic
try {
    foreach (order_approval_pending_items($obconn, $dpconn ?? null) as $cartItem) {
        $approvalItems[] = $cartItem;
    }
} catch (PDOException $e) {
    // Table may not exist yet; silently continue
}
...
$focItemsByClaim = function_exists('foc_claim_items_for_claims')
    ? foc_claim_items_for_claims($obconn, $focClaimMap)
    : [];
$servicePartsByComplaint = function_exists('complaint_service_log_parts_for_complaints')
    ? complaint_service_log_parts_for_complaints($obconn, $serviceComplaintIds)
    : [];
foreach ($approvalItems as &$item) {
    // APPROVAL MODULE: Added Level 1 and Level 2 cart approval logic
    ...
    if (function_exists('installed_base_fab_link_html')) {
        $item['fab_html'] = installed_base_fab_link_html($obconn, (string) ($item['fab_number'] ?? ''), $installedBaseIdByFab);
    } else {
        $item['fab_html'] = htmlspecialchars((string) ($item['fab_number'] ?? ''), ENT_QUOTES, 'UTF-8');
    }
    if (($item['claim_type'] ?? '') === 'foc') {
        $item['details_html'] = function_exists('foc_parts_linked_summary_html')
            ? foc_parts_linked_summary_html($focItemsByClaim[(int) $item['id']] ?? [])
            : htmlspecialchars((string) ($item['details'] ?? ''), ENT_QUOTES, 'UTF-8');
        $item['parts_html'] = $item['details_html'];
        continue;
    }
    $item['details_html'] = '';
    $item['parts_html'] = function_exists('foc_parts_linked_summary_html')
        ? foc_parts_linked_summary_html(
        ...
        <div class="page-header">
            <div>
                <div class="page-subtitle">
                    Pending orders that contain Level 1 Approval Price or Level 2 Approval Price products.
                </div>
            </div>
        </div>
            ...
            <div class="complaint-form-header">
                <div class="complaint-form-header__main">
                    ...
                    <div>
                        ...
                        <p class="complaint-form-header__subtitle">Orders waiting for Level 1 or Level 2 approval. AO Number is generated only after required approvals are completed.</p>
                    </div>
                </div>
            </div>
                    ...
                    <table id="approvalsTable" class="table table-hover booking-table w-100">
                        <thead>
                            <tr>
                                <th width="6%">#</th>
                                <th width="10%">Type</th>
                                <?php /* <th width="10%">Call Ticket</th> */ ?>
                            ...
                            <tr>
                                ...
                                */ ?>
                                <td>
                                    ...
                                    <?php elseif (function_exists('installed_base_fab_link_html')): ?>
                                        <?= installed_base_fab_link_html($obconn, (string) ($row['fab_number'] ?? ''), $installedBaseIdByFab) ?>
                                    <?php else: ?>
                                        <?= htmlspecialchars((string) ($row['fab_number'] ?? '-')) ?>
                                    <?php endif; ?>
                                </td>
...
(function () {
    ...
    if (typeof $.fn.DataTable !== 'undefined' && document.getElementById('approvalsTable')) {
        $('#approvalsTable').DataTable({
            ...
            language: { emptyTable: 'There are no pending orders waiting for Level 1 or Level 2 approval.' }
        });
    }
