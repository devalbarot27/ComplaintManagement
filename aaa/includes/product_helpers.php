...
function product_ensure_schema(PDO $conn): void
{
    ...
    $conn->exec('
        ALTER TABLE product_master_vayu
        ADD COLUMN IF NOT EXISTS level_1_approval_price NUMERIC NULL
    ');
    $conn->exec('
        ALTER TABLE product_master_vayu
        ADD COLUMN IF NOT EXISTS level_2_approval_price NUMERIC NULL
    ');
...
function product_from_post(array $post, ?array $existing = null): array
{
    $dealerPrice = array_key_exists('dealer_price', $post)
        ? trim((string) $post['dealer_price'])
        : trim((string) ($existing['dealer_price'] ?? ''));
    ...
    $data = [
        ...
        'level_1_approval_price' => trim((string) ($post['level_1_approval_price'] ?? '')),
        'level_2_approval_price' => trim((string) ($post['level_2_approval_price'] ?? '')),
    ...
    if ((int) $data['order_type'] === 2) {
        $data['level_1_approval_price'] = '';
        $data['level_2_approval_price'] = '';
    }
...
function product_validate(array $data): ?string
{
    ...
    if ((int) ($data['order_type'] ?? 0) !== 2) {
        if ($data['level_1_approval_price'] === '') {
            return 'Level 1 Approval Price is required.';
        }
        if (!product_is_numeric_value($data['level_1_approval_price'])) {
            return 'Level 1 Approval Price must be numeric.';
        }

        if ($data['level_2_approval_price'] === '') {
            return 'Level 2 Approval Price is required.';
        }
        if (!product_is_numeric_value($data['level_2_approval_price'])) {
            return 'Level 2 Approval Price must be numeric.';
        }
    }
...
function product_search_filter(string $searchValue): array
{
    return rbac_search_filter($searchValue, [
        ...
        'tpldesc',
        'dealer_price',
        'CAST(level_1_approval_price AS TEXT)',
        'CAST(level_2_approval_price AS TEXT)',
        'tod_flag',
        'excisable',
        'CAST(mc AS TEXT)',
        'CAST(vc AS TEXT)',
        ...
        'CASE order_type WHEN 1 THEN \'Units\' WHEN 2 THEN \'Spares\' ELSE CAST(order_type AS TEXT) END',
...
function product_updated_by_display_name(PDO $conn, ?string $username): string
{
    $username = trim((string) $username);
    if ($username === '') {
        return '-';
    ...
    $stmt = $conn->prepare('
        SELECT COALESCE(NULLIF(TRIM(name), \'\'), NULLIF(TRIM(username), \'\')) AS display_name
        FROM user_master
...
function product_bind_common(PDOStatement $stmt, array $data): void
{
    ...
    $stmt->bindValue(':level_1_approval_price', product_nullable_numeric($data['level_1_approval_price'] ?? ''));
    $stmt->bindValue(':level_2_approval_price', product_nullable_numeric($data['level_2_approval_price'] ?? ''));
...
function product_insert(PDO $conn, array $data, ?string $updatedBy): void
{
    ...
    $stmt = $conn->prepare('
        INSERT INTO product_master_vayu (
            ...
            level_1_approval_price, level_2_approval_price,
...
function product_update(PDO $conn, int $id, array $data, ?string $updatedBy): void
{
    ...
    $stmt = $conn->prepare('
        UPDATE product_master_vayu SET
            ...
            level_1_approval_price = :level_1_approval_price,
            level_2_approval_price = :level_2_approval_price,
            tod_flag = :tod_flag,
            excisable = :excisable,
            mc = :mc,
            vc = :vc,
