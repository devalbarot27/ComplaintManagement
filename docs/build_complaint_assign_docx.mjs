...
const mdPath = path.join(__dirname, 'LLD_Complaint_Entry_Assign_Process.md');
const outPath = path.join(__dirname, 'LLD_Complaint_Entry_Assign_Process.docx');
const diagramsDir = path.join(__dirname, 'diagrams_complaint_assign');

const titles = [
  'Figure 1 â€” High-level architecture',
  'Figure 2 â€” Entity relationship diagram',
  'Figure 3 â€” Create database flow',
  'Figure 4 â€” Sequence: Create / update on Spare Parts page',
  'Figure 5 â€” Sequence: Add Spare Parts from IB / SL row',
  'Figure 6 â€” Activity diagram',
  'Figure 7 â€” Class / module diagram',
];
    ...
    case 'heading':
      return [
        new Paragraph({
          ...
          children: [new TextRun({ text: (node.children || []).map(cellText).join('') })],
        }),
      ];
    case 'paragraph': {
...
const doc = new Document({
  ...
  title: 'LLD â€” Complaint Entry and Assign Process',
  description: 'Low-Level Design for Complaint Entry and Assign Process',
  sections: [
    {
