...
const mdPath = path.join(__dirname, 'LLD_Assign_Permissions_Module.md');
const outPath = path.join(__dirname, 'LLD_Assign_Permissions_Module.docx');
const diagramsDir = path.join(__dirname, 'diagrams_assign_permissions');

const titles = [
  'Figure 1 — High-level architecture',
  'Figure 2 — Entity relationship diagram',
  'Figure 3 — Save assignment database flow',
  'Figure 4 — Sequence: Load matrix',
  'Figure 5 — Sequence: Save assignment',
...
function blockToChildren(node) {
  switch (node.type) {
    case 'heading': {
      ...
      // Force bold for main LLD sections 1–18 (## headings like "1. Module Overview").
      const isNumberedSection =
        (node.depth || 1) === 2 && /^(?:\*\*)?\d{1,2}\.\s/.test(text);
      return [
        new Paragraph({
...
const doc = new Document({
  ...
  title: 'LLD — Assign Permissions Module',
  description: 'Low-Level Design for Assign Permissions Module',
  sections: [
    {
