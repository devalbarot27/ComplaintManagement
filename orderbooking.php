...
<body>
    <div class="main-wrapper" id="mainWrapper">
        ...
        <div class="content">
                ...
                <div class="order-form-grid" id="orderBookingForm">
 				...
 				<option value="">Select</option>
                    ...
                    <div class="form-group delivery-date-group">
                        ...
                        <input type="text" class="form-control" id="dDate" placeholder="Delivery Date" autocomplete="false" readonly/>

                    </div>
                    <input type="hidden" name="deliveryTerm" value="CIF" />
                    <!--<div class="form-group">
            ...
            <div class="order-form-card" id="orderFormCard">
                <div class="order-form-grid order-form-grid--add-item">
                    <div class="form-group order-form-grid--add-item__product">
                    ...
                    <div class="form-group js-units-price-field">
                        <label>CLP - Customer List Price</label>
                        <input type="text" class="custom-input" id="clpPrice" readonly>
                    </div>

                    <div class="form-group js-units-price-field">
                        <label>Dealer Price <span class="text-danger">*</span></label>
                        <input type="text" class="custom-input" id="dealerPrice" placeholder="0.00" inputmode="decimal" autocomplete="off">
                        <div id="dealerPriceError" class="text-danger small mt-1" style="display:none;">Price should not be accepted.</div>
                    </div>

                    <input type="hidden" id="l1Price" value="">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="custom-input" id="unitPrice" readonly>
                    </div>
                </div>
                <div id="l2ApprovalNote" class="delivery-date-note" style="display:none;" role="status">
                    <i class="bi bi-info-circle" aria-hidden="true"></i>
                    <span id="approvalLevelNoteText">This order will go for Level 1 approval.</span>
                </div>
                <div class="add-btn-wrapper">
                    <button type="button" class="add-item-btn" id="addItemBtn" disabled onclick="addItemToCart()">
            ...
            <div class="booking-card cart-section" id="divCartHeader">
                ...
                <div class="cart-section__footer">
                    ...
                    <div class="cart-section__actions">
                        <button type="button" class="add-item-btn cart-submit-btn d-none" id="divbtnUpload1" style="display:none" onclick="submitCartApi()">
                        ...
                        <button type="button" class="add-item-btn cart-submit-btn" id="divbtnApproval" style="display:none" onclick="submitCartForApproval()">
                            <i class="bi bi-check2-circle"></i> Submit Order
                        </button>
                    </div>
                </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $("#dealerPrice").on("input", function() {
            ...
            $(this).val(value);
            updateUnitPrice();
            validateDealerPriceAgainstL2();
        });
        $("#dealerPrice").on("blur", function() {
        ...
        $("#qty").on("input change", function() {
            updateUnitPrice();
            if (isSparesOrderType()) {
                validateSparesAddItem();
            }
        });

        $("#orderType").on("change", function() {
            var selected = String($(this).val() || '');
            var cartRow = document.querySelector('#cartTable tbody tr.cart-item-row');
            var cartType = cartRow ? String(cartRow.getAttribute('data-order-type') || '') : '';
            if (cartType && cartType !== selected) {
                $(this).val(cartType);
                alert('Items with different order types cannot be added to the same cart. Please clear the cart or use the existing order type.');
                syncOrderTypeProductUi();
                return;
            }
    ...
    function getCatalogClpPrice() {
        var prices = window.currentProductPrices || {};
        var clp = parseFloat(prices.clp);
        return isNaN(clp) ? 0 : clp;
    }

    function getCatalogL1Price() {
        var prices = window.currentProductPrices || {};
        var l1 = parseFloat(prices.level_1);
        if (isNaN(l1) || l1 <= 0) {
            l1 = parseFloat($("#l1Price").val());
        }
        return isNaN(l1) ? 0 : l1;
    }

    function getCatalogL2Price() {
        var prices = window.currentProductPrices || {};
        var l2 = parseFloat(prices.level_2);
        return isNaN(l2) ? 0 : l2;
    }

    function isSparesOrderType() {
        return String($("#orderType").val() || '') === '2';
    }

    function cartIsSparesOrder() {
        var row = document.querySelector('#cartTable tbody tr.cart-item-row');
        if (!row) {
            return isSparesOrderType();
        }
        return String(row.getAttribute('data-order-type') || '') === '2';
    }

    function syncOrderTypeProductUi() {
        var spares = isSparesOrderType();
        var grid = document.querySelector('.order-form-grid--add-item');
        if (grid) {
            grid.classList.toggle('is-spares', spares);
        }
        if (spares) {
            var err = document.getElementById('dealerPriceError');
            if (err) {
                err.style.display = 'none';
            }
            var note = document.getElementById('l2ApprovalNote');
            if (note) {
        ...
        updateCartSubmitButtonLabel();
    }

    function validateSparesAddItem() {
        var item = ($("#item").val() || '').trim();
        var hasItem = item && item !== 'Select a product';
        var clp = getCatalogClpPrice();
        if (clp <= 0) {
            clp = parseFloat($("#clpPrice").val());
        }
        var qty = parseFloat($("#qty").val());
        var hasClp = !isNaN(clp) && clp > 0;
        var hasQty = !isNaN(qty) && qty > 0;
        setAddItemEnabled(hasItem && hasClp && hasQty);
        return hasClp;
    }

    function isDealerPriceZeroOrNegative(dealerPrice) {
        return !isNaN(dealerPrice) && dealerPrice <= 0;
    }

    function isDealerPriceAboveClp(dealerPrice) {
        var clp = getCatalogClpPrice();
        if (clp <= 0 || isNaN(dealerPrice)) {
            return false;
        }
        return dealerPrice > clp;
    }

    function isDealerPriceBelowL2(dealerPrice) {
        var l2 = getCatalogL2Price();
        if (isNaN(dealerPrice) || l2 <= 0) {
            return false;
        }
        return dealerPrice < l2;
    }

    function isDealerPriceRejected(dealerPrice) {
        return isDealerPriceZeroOrNegative(dealerPrice)
            || isDealerPriceAboveClp(dealerPrice)
            || isDealerPriceBelowL2(dealerPrice);
    }

    function isL2ApprovalRequiredForDealerPrice(dealerPrice) {
        var l1 = getCatalogL1Price();
        if (isNaN(dealerPrice) || l1 <= 0) {
            return false;
        }
        return dealerPrice < l1;
    }

    function updateL2ApprovalNote() {
        var note = document.getElementById('l2ApprovalNote');
        if (note) {
            note.style.display = 'none';
        }
    ...
    function validateDealerPriceAgainstL2() {
        if (isSparesOrderType()) {
            var err = document.getElementById('dealerPriceError');
            if (err) {
                err.style.display = 'none';
            }
            return validateSparesAddItem();
        }
        var raw = ($("#dealerPrice").val() || '').trim();
        var dealer = parseFloat(raw);
        var invalid = isDealerPriceRejected(dealer);
        var err = document.getElementById('dealerPriceError');
        if (err) {
        ...
        var item = ($("#item").val() || '').trim();
        var hasItem = item && item !== 'Select a product';
        var hasValidPrice = raw !== '' && !isNaN(dealer) && !invalid;
        setAddItemEnabled(hasItem && hasValidPrice);
        updateL2ApprovalNote();
        return !invalid;
    }

    function updateUnitPrice() {
        if (isSparesOrderType()) {
            var clp = parseFloat($("#clpPrice").val());
            var qty = parseFloat($("#qty").val());
            if (isNaN(clp) || clp <= 0 || isNaN(qty) || qty < 0) {
                $("#unitPrice").val("");
                return;
            }
            $("#unitPrice").val((qty * clp).toFixed(2));
            return;
        }
        var dealer = parseFloat($("#dealerPrice").val());
        var qty = parseFloat($("#qty").val());
        if (isNaN(dealer) || isNaN(qty) || qty < 0) {
            $("#unitPrice").val("");
            return;
        }
        $("#unitPrice").val((dealer * qty).toFixed(2));
    }

    function clearProductPrices() {
        $("#clpPrice").val("");
        $("#dealerPrice").val("");
        $("#l1Price").val("");
        $("#unitPrice").val("");
        window.currentProductPrices = null;
        var err = document.getElementById('dealerPriceError');
        if (err) {
            err.style.display = 'none';
        }
    ...
    function fillProductPrices(prices) {
        prices = prices || {};
        var clp = parseFloat(prices.clp);
        var l1 = parseFloat(prices.level_1);
        var dealer = l1;

        $("#clpPrice").val(formatCatalogPrice(clp));
        $("#l1Price").val(formatCatalogPrice(l1));

        if (isNaN(dealer) || dealer <= 0) {
            dealer = clp;
        }
        $("#dealerPrice").val(formatCatalogPrice(dealer));
        updateUnitPrice();
        if (isSparesOrderType()) {
            validateSparesAddItem();
            return;
    ...
    function enableBtn() {
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res.status == true) {
                    window.currentProductPrices = res.prices || null;
                    fillProductPrices(window.currentProductPrices);
                } else {
                    clearProductPrices();
                    alert(res.message);
                }
    ...
    function addItemToCart() {
        ...
        var spares = isSparesOrderType();
        var price = spares ? $("#clpPrice").val().trim() : $("#dealerPrice").val().trim();
        var orderType = $("#orderType").val();
        ...
        const fields = [{
            ...
            {
                value: price,
                message: spares ? "Please select a product with a valid CLP" : "Please enter a dealer price"
            },
            {
                value: orderType,
        ...
        if (spares) {
            if (!validateSparesAddItem()) {
                return;
            }
        } else if (!validateDealerPriceAgainstL2()) {
            return;
        }
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res == '1') {
                    ...
                    if (res === 'Price should not be accepted.') {
                        validateDealerPriceAgainstL2();
                        return;
                    }
                    alert(res);
    ...
    function getItems() {
        $.ajax({
            ...
            success: function(response) {
                ...
                var cartRow = document.querySelector('#cartTable tbody tr.cart-item-row');
                var cartType = cartRow ? String(cartRow.getAttribute('data-order-type') || '') : '';
                if (cartType && $("#orderType").val() !== cartType) {
                    $("#orderType").val(cartType);
                }
                syncOrderTypeProductUi();
                (tCount > 0) ? setCartActionButtonsVisible(true) : setCartActionButtonsVisible(false);
                $("#divCartHeader").show();
    ...
    function getCartApprovalSubmitBlockMessage() {
        const pending = [];
        const rejected = [];

        document.querySelectorAll('#cartTable tbody tr.cart-item-row').forEach(function(row) {
            const priceType = (row.getAttribute('data-price-type') || 'clp').toLowerCase();
            if (priceType !== 'level_1' && priceType !== 'level_2') {
                return;
            }

            const status = (row.getAttribute('data-approval-status') || 'pending').toLowerCase();
            if (status === 'approved') {
                return;
            }

            const itemCode = (row.getAttribute('data-item-code') || '').trim() || 'Unknown item';
            const levelLabel = priceType === 'level_2' ? 'Level 2 Approval' : 'Level 1 Approval';
            const entry = itemCode + ' (' + levelLabel + ')';
        ...
        const messages = [];
        if (pending.length) {
            messages.push('Cannot submit the order. Pending approval: ' + pending.join(', ') + '. Please wait for approval or remove rejected items before submitting.');
        }
        if (rejected.length) {
            messages.push('Cannot submit the order. Rejected: ' + rejected.join(', ') + '. Please wait for approval or remove rejected items before submitting.');
        }

        return messages.join('\n');
    }

    function blockSubmitIfCartApprovalPending() {
        const message = getCartApprovalSubmitBlockMessage();
        if (message) {
            alert(message);
            return true;
        }
    ...
    function cartHasApprovalPriceItems() {
        ...
        return document.querySelectorAll('#cartTable tbody tr.cart-item-row').length > 0;
    }

    function updateCartSubmitButtonLabel() {
        const needsApproval = cartHasApprovalPriceItems();
        const $btn = $("#divbtnApproval");
        $btn.html(needsApproval
            ? '<i class="bi bi-check2-square"></i> Submit for Approval'
            : '<i class="bi bi-check2-circle"></i> Submit Order');
    }

    function setCartActionButtonsVisible(visible) {
        $("#divbtnUpload").hide();
        $("#divbtnUpload1").hide();
        if (visible) {
            updateCartSubmitButtonLabel();
    ...
    function submitCart() {
        if (blockSubmitIfCartApprovalPending()) {
            return;
        }
        $("#loader").show();
        setCartActionButtonsVisible(false);
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res.status == "success") {
                    ...
                    $("#loader").hide();
                    setCartActionButtonsVisible(true);
                    alert((res && res.message) ? res.message : "Error in placing order ! Please contact IT");
                }
            }
        });
    }
    ...
    // APPROVAL MODULE: Save order without AO number and send items for approval
    function submitCartForApproval() {
        $("#loader").show();
        setCartActionButtonsVisible(false);
        var orderCategory = $("#orderCategory").val().trim();
        var addressCode = $("#customer_master").val().trim();
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res.status === "success") {
                    ...
                    $("#fAmount").val("");
                    $("#loader").hide();
                    var needsApproval = res.needs_approval === true || res.needs_approval === 1 || res.needs_approval === "1";
                    SuccessModal.show({
                        title: needsApproval ? 'Order Sent for Approval' : 'Order Submitted Successfully',
                        message: res.message
                            ? res.message
                            : (needsApproval
                                ? 'Your order has been saved without an AO Number and sent for Level 1 approval.'
                                : 'Order submitted successfully. AO Number generation has been requested from LN.'),
                        status: res.order_no,
                        onClose: function() {
                            window.location.href = "recent_orders.php?order_no=" + res.order_no;
                        }
